...
module.exports = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (amount && isNaN(amount)) {
      await interaction.reply({ content: "Please insert a valid amount between 1 to 100.", ephemeral: true });
    }
    ...
    try {
      ...
      interaction.reply({ content: `Deleted ${bulkDel.size} messages.`, ephemeral: true });
    } catch (err) {
      console.log("Clear command err: ", err);
      interaction.reply({ content: `There was an error deleting the messages.`, ephemeral: true });
    }
  },
};
